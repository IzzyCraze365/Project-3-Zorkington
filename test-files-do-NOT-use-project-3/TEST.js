...
async function start() {
  hero.name = await introduction(); //The player will have to name themselves;
  //console.log(hero.name, userInput, "Before Loop of Function\n"); //! TEST
  colorChangeWords(`\nYou, ${hero.name} find yourself at the Beginning of a Grand Adventure!\nAnd it all starts right here in this quaint little village.\nIt is probably a good idea to "Look" around.\n(type "Help" to see a list of available actions.)`,highlightedWords);
  while (userInput !== "Exit") {
    colorChangeWords(
      `\nYou are currently in the ${currentLocation}.`,
      highlightedWords
    );
    userInput = await heroAction();
    //console.log(hero.name, userInput, "Inside Loop of Function\n"); //! TEST
  }
}

async function heroAction() {
  const heroAction = `What would you like to do, ${hero.name}?\n>_ `;
  ...
  if (action === "Exit" || action === "E") {
   quitGame();
    ...
    if (hero.inventory.includes(droppingItem) === true) {
      ...
      colorChangeWords(
        `\nSorry ${hero.name}, you don't have a ${dropItem} to drop.\n`,
        highlightedWords
      );
    }
    ...
    if (locations[currentLocation].inventory.includes(tookenItem) === true) {
      ...
      colorChangeWords(
        `\nSorry ${hero.name}, there is no ${takeItem} for you to take.\n`,
        highlightedWords
      );
    }
    ...
    if ((locations[currentLocation].inventory.includes(interactableObject) === true) ||(hero.inventory.includes(interactableObject))===true) {
      ...
      interactCommodity[interactableObject].followUp();}
        ...
        interactPeople[interactableObject].followUp();}
          ...
          colorChangeWords(
            `\nSorry ${hero.name}.  You can't interact with ${interactObject}.\n`,highlightedWords);
        }
...
function quitGame(){
  colorChangeWords(
    `\nThis is where the Adventure of ${hero.name} comes to an end.\n`,
    highlightedWords
  );
  process.exit();
}
...
function unknownPrompt(input) {
  colorChangeWords(
    `\nSorry ${hero.name}, you don't know how to ${input}.\n`,
    highlightedWords
  );
}
